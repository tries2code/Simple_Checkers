...
string text;											//Nimmt den Regel-Text auf 
...
class Checkers_window : public Window {
...
public:
	Checkers_window(Point xy, int w, int h, const string& title);
...
private:
	...
	vector<Point> strike();
	Graph_lib::Circle* get_stone(Point& p);
	void make_king(Graph_lib::Circle* c);
	bool is_king(Graph_lib::Circle* c);

	static void cb_rules(Address, Address addr) { static_cast<Checkers_window*>(addr)->rules(); }		//Sogenannte Call Back Funktionen für Knöpfe, callen die eigentlichen Funktionen für die Knöpfe
	static void cb_restart(Address, Address addr) { static_cast<Checkers_window*>(addr)->restart_game(); }
	static void cb_quit(Address, Address addr) { reference_to<Checkers_window>(addr).quit(); }		//Ist die Stroustrup Variante, macht auch nur 		return *static_cast<W*>(pw);
	static void cb_tile_pressed(Address, Address addr) { reference_to<Checkers_window>(addr).tile_pressed(); }
...
Checkers_window::Checkers_window(Point xy, int w, int h, const string& title) :Window(w, h, title),
...
void Checkers_window::wait_for_button() {
	while (!button_pushed) Fl::wait();
	button_pushed = false;
	Fl::redraw();
}

bool Checkers_window::tile_empty(Point p)const {
...
bool Checkers_window::hostile_present(Point p) const {
...
bool Checkers_window::must_attack(Point p) const {
...
vector<Point> Checkers_window::strike() {
...
Graph_lib::Circle* Checkers_window::get_stone(Point& p) {				//Ermittelt Spielstein
...
void Checkers_window::make_king(Graph_lib::Circle* c) {					//Spielstein wird zur Dame wenn er das gegenüberliegende Ende erreicht
	int y = c->center().y - ca;
	if (c->fill_color() == c_player1 && y == us)c->set_style(king_style);
	if (c->fill_color() == c_player2 && y == us + sz * 7)c->set_style(king_style);
}
bool Checkers_window::is_king(Graph_lib::Circle* c) {					//Ist der Speilstein eine Dame?
	if (c->style() == Line_style(Line_style::dash, 8))return true;
	return false;
}

void Checkers_window::restart_game() {
...
void  Checkers_window::tile_pressed() {
	...
	if (tile_empty(p) && stone_selected && is_king(curr_stone)) {					//Dame bewegen und fangen
		...
		if (tile_empty({ temp_x,temp_y }))move_to = { temp_x,temp_y };
...
int main() {
	...
	Checkers_window win{ {100,100},screen_x,screen_y,"Schach oder Dame" };						//Fenster zum Debuggen verkleinert um das Konsolenfenster zu sehen
